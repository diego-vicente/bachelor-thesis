...
\item \textbf{Left identity}: this law states that putting a pure value in
  ...
  This formal proof for this law is covered in Listing \ref{monad:leftid}.

\begin{lstlisting}[style = haskell, caption = Left identity formal proof for
\texttt{SearchM}, label = monad:leftid]
return x >>= f == f x
-- { definition of return, where s :: Statistics with identity value }
SearchM x s >>= f == f x
-- { definition of >>= }
let SearchM x' s' = f x in SearchM x' (mergeStats s s') == f x
-- { definition of f }
SearchM x' (mergeStats s s') == SearchM x' s'
-- { applying identity of s }
